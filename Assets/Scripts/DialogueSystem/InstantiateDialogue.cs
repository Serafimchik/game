...
public class InstantiateDialogue : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Vector3.Distance(player.position, objectToShow.position) < proximityDistance && !dialogueEnded)
        {
            objectToShow.gameObject.SetActive(true); // Отображаем объект, если условия выполняются
        }
        else
        {
            objectToShow.gameObject.SetActive(false); // Скрываем объект, если условия не выполняются
        }
    }
    */
